...
const HEADLESS_APP_ROW_IDS = ["headless-startup", "headless-runner"];
...
function hostVerificationOverlay(input) {
	...
	return [...HEADLESS_APP_ROW_IDS.map((id) => ({
		id,
		disabled: true
	})), { insert: [{
...
function packageEntries(ctx, packageName) {
...
async function waitForLoader(ctx, packageName) {
	const loader = contextLoader(ctx);
	if (!loader) return {
		stable: false,
		sourceMatched: false,
...
//#endregion
export { hashObject as _, hostVerificationOverlay as a, selectInstallVerificationLayer as c, FORGED_RESUME_HOST_KEYS as d, POLICY_VERSION as f, classifyRuntimeSurface as g, VERIFICATION_STATUSES as h, hostLayerSuccess as i, verificationChildEnv as l, VERIFICATION_LAYER_KINDS as m, declaredVerificationFixturesFromPackage as n, inspectLoadedToolSafety as o, TOOL_NAMES as p, fixtureDigestFor as r, sanitizeHostVerificationEvidence as s, applyHostVerification as t, BRIDGE_EXECUTION_TOOLS as u, sha256 as v };
