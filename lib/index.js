...
var ExecutionGuard = class {
	...
	constructorDenial(name, exec) {
		...
		if (matchesSet(name, SHELL_TOOLS)) {
			...
			if (GIT_PUBLICATION_OR_DESTRUCTIVE_RE.test(command) || GH_PUBLICATION_RE.test(command)) return "Managed construction requires a fresh user decision before publication or destructive repository operations.";
		}
		if (matchesSet(name, FS_WRITE_TOOLS)) {
...
async function disableBuiltinMount(input) {
	...
	if (rows.filter((row) => exactBuiltinRow(row, input.spec.mountId, input.packageName)).length !== 1) throw new EvolutionError("review_expired", "The exact built-in mount row changed after enablement; refusing removal");
