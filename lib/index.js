...
function shellForwardedFileSpec$1(filename) {
...
async function npmPackArgv$1(runner, signal) {
...
async function freezePackedSource(sourceRoot, options) {
	...
	try {
		const [npmCommand, ...npmPrefix] = await npmPackArgv$1(options.runner, options.signal);
	...
	return {
		installSpec: shellForwardedFileSpec$1(tarball),
		artifactRoot,
...
async function freezeGithubPackage(options) {
	...
	try {
		...
		if ((await runChecked(options.runner, [
			git,
			"rev-parse",
			"HEAD"
		], sourceRoot, options)).trim().toLowerCase() !== options.commit.toLowerCase()) throw new EvolutionError("review_rejected", "Frozen GitHub source HEAD does not match the requested commit");
		if ((await runChecked(options.runner, [
			git,
			"status",
			"--porcelain"
		], sourceRoot, options)).trim()) throw new EvolutionError("review_rejected", "Frozen GitHub source is not clean after checkout");
...
async function previewGithubPlugin(options) {
	const result = await githubSnapshot({
		...options,
		preview: true
	});
...
var ExecutionGuard = class {
	...
	parentDenial(name, exec) {
		...
		if (matchesSet(name, SHELL_TOOLS)) {
			...
			if (!isSafeShellCommand(command, SAFE_PARENT_SHELL_RE)) return "Capability Evolution Policy V13 permits only allowlisted read-only shell inspection commands before managed construction.";
		}
	}
...
function buildManifest(files, templateVersion = "17") {
	const ordered = {};
...
async function materializeEvolutionPresetLocked(options, paths, presetsRoot, targetDir, renamePath) {
	const templateVersion = options.templateVersion ?? "17";
...
var PluginInstaller = class {
	...
	async install(input, exec, binding) {
		...
		const lockfileBefore = input.retention === "persistent" ? await readFile(path.join(dshHome, "profiles", input.targetProfile, "pnpm-lock.yaml"), "utf8").catch(() => void 0) : void 0;
		try {
			await this.installWithTransientRetry({
				dshHome,
...
function createTools(service) {
	return [
		defineTool({
			...
			description: "Start autonomous capability discovery. Host preserves the current top-level user message as the authoritative requirement; requirement is only a non-authoritative summary. The Agent must supply one to five complementary exact GitHub search phrases, each containing one or two search terms. GitHub ANDs the terms in each phrase, so never pass a prose request or pack many concepts into one query. Host only normalizes, validates, scopes, and executes the phrases; it never silently rewrites or replans them. If Host rejects query shape, correct the arguments and retry in the same user turn; rejected queries consume no search quota. If one material clarification is required, omit queries until the fresh answer and provide replacement queries through clarify_requirement.",
			parameters: {
				requirement: {
					type: "string",
					required: true,
				...
				queries: {
					type: "array",
					items: { type: "string" },
					description: "One to five complementary exact GitHub search phrases generated by the Agent, for example \"auto approve\" and \"approval automation\". Each phrase contains one or two terms because GitHub ANDs its terms. Omit only when asking one material clarification."
				},
		...
		defineTool({
			...
			description: "Refine an open discovery pool with Agent-planned complementary exact query phrases or strict owner/repository identities. Each query contains one or two terms because GitHub ANDs its terms. Host validates and executes these parameters unchanged. Correct a rejected query plan in the same user turn; rejection consumes no search quota. This is read-only and cannot seal Gate 1, review, install, modify, or create.",
			parameters: {
				workflow_id: {
					type: "string",
					required: true
				...
				navigation: {
					...
					properties: {
						...
						queries: {
							type: "array",
							items: { type: "string" },
							description: "One to five fresh complementary exact search phrases planned by the Agent from the user reply, each containing one or two terms because GitHub ANDs its terms. Legal for clarify_requirement or search_more."
						},
						repositories: {
							type: "array",
							items: { type: "string" },
