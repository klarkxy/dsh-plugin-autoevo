...
var PluginInstaller = class {
	...
	async installWithTransientRetry(input) {
		...
		try {
			...
			const absent = await this.launcher.profileTargetAbsent(input.dshHome, input.profile, input.packageName).catch(() => false);
			input.signal?.throwIfAborted();
...
var StateStore = class {
	...
	strictDiagnosticKeys = /* @__PURE__ */ new Set();
	constructor(root) {
	...
	recordDiagnostic(kind, fileName, error, strict = false) {
	...
	async listPass(kind, strictValidator) {
		...
		for (const key of this.diagnostics.keys()) {
			...
			} else if (!this.strictDiagnosticKeys.has(key)) this.diagnostics.delete(key);
		}
		let entries;
		try {
		...
		for (const diagnostic of diagnostics) this.diagnostics.set(`${kind}/${diagnostic.fileName}`, diagnostic);
		return {
			records,
			diagnostics
		};
