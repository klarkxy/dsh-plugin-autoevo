...
async function recoverStaleLock(lockPath, observed, testing, signal) {
	...
	try {
		if (await lstat(quarantine).then(() => true).catch((error) => {
...
async function acquireLock(lockPath, timeoutMs, signal, testing = {}) {
	...
	while (true) {
		...
		try {
			...
			if (error instanceof LockBusyError) {} else if (!error || typeof error !== "object" || !("code" in error) || error.code !== "EEXIST") throw error;
		}
		...
		if ((!owner?.pid || !(testing.processAlive ?? processAlive)(owner.pid)) && age > Math.max(5e3, timeoutMs)) try {
...
function receiptOwnedRoots(stateRoot) {
	...
	try {
		return readdirSync(directory).filter((entry) => /^installation_[a-f0-9]{16,64}\.json$/u.test(entry)).map((entry) => path.join(directory, entry)).flatMap((filePath) => {
			try {
...
async function resolveProfilePluginCapabilities(input) {
	...
	if (!within$1(home, profileRoot)) return [];
	const profileManifestPath = path.resolve(profileRoot, "package.json");
	if (!within$1(profileRoot, profileManifestPath)) return [];
...
var PluginInstaller = class {
	...
	hotLoader;
	constructor(ctx, config, store, launcher, _revalidate, authorizeInstall, hotLoader, semanticVerifier, preflightProfile, resolveDestinationProfile) {
		this.ctx = ctx;
		this.config = config;
		this.store = store;
		this.launcher = launcher;
		this._revalidate = _revalidate;
	...
	async assertPersistentDestination(input, packageName, signal) {
		...
		let absent;
		try {
			absent = await this.launcher.profileTargetAbsent(this.config.dshHome, input.targetProfile, packageName);
		} catch (error) {
			if (signal?.aborted) throw signal.reason;
			throw error;
		}
	...
	async reconcileReplacement(input) {
		...
		if (this.launcher.profileDependencySpec) try {
	...
	async requestInstallApproval(attempt) {
		...
		try {
			...
			if (input.retention === "temporary") await this.removeOwnedDirectory(trialRoot, trialsRoot).catch(() => void 0);
			if (exec.signal?.aborted) throw exec.signal.reason;
			throw error;
		}
	}
	...
	async writeProvisionalReceipt(attempt) {
		const { input, binding, review, id, createdAt, packageName, installSpec, artifactSha256, dshHome, trialRoot, trialsRoot, exec } = attempt;
		exec.signal?.throwIfAborted();
		const provisional = {
			schemaVersion: 2,
			id,
	...
	async settleInterruptedAttempt(attempt, stage) {
		const { input, review, packageName, installSpec, dshHome, trialRoot, trialsRoot, provisional, destinationJournal } = attempt;
		let removed = false;
		let cleanupFailed = false;
		let installState = "unknown";
		let replacementJournal = destinationJournal?.replacement;
		if (stage === "preflight" || input.retention === "temporary") try {
			...
			try {
				exactSourcePresent = await this.launcher.profileSourceMatches(dshHome, input.targetProfile, packageName, installSpec);
			} catch {
				exactSourcePresent = false;
			}
			if (exactSourcePresent) installState = "installed";
			else try {
				installState = await this.launcher.profileTargetAbsent(dshHome, input.targetProfile, packageName) ? "not_installed" : "unknown";
			} catch {
				installState = "unknown";
			}
		}
		const installOutcome = stage === "hotload" ? "recovery_required" : installState === "not_installed" && !cleanupFailed ? "failed_absent" : "recovery_required";
		const failure = lifecycleFailure(stage === "preflight" ? "preflight" : stage === "hotload" ? "load" : "install", "operation_cancelled", cleanupFailed ? "Installation was cancelled while an effect was in flight, and owned cleanup could not be confirmed." : stage === "hotload" ? "Installation was cancelled while current-process activation may have mutated runtime state." : "Installation was cancelled while an install effect was in flight; Host state was reconciled before returning.", false);
	...
	async runIsolatedPreflight(attempt) {
		...
		try {
			preflightSourceMatched = await this.launcher.profileSourceMatches(preflightHome, this.preflightProfile, packageName, installSpec);
		} catch (error) {
			if (exec.signal?.aborted) {
				await this.settleInterruptedAttempt(attempt, "preflight");
				throw exec.signal.reason;
		...
		const preflightRecord = {
			...
			...cleanPreflightPassed ? {} : { installFailure: lifecycleFailure("preflight", !removed ? "cleanup_failed" : preflightSourceMatched ? "verification_failed" : "source_mismatch", !removed ? "Isolated preflight completed, but the owned trial could not be removed; recovery is required." : preflightSourceMatched ? "Isolated preflight did not prove the frozen verification layer." : "Isolated preflight did not activate the exact reviewed source.") },
			preflight: {
				profile: this.preflightProfile,
	...
	async activateInstalledBundle(attempt) {
		...
		if (exec.signal?.aborted) {
			attempt.hotReloadAttempt = input.retention === "persistent" && awaitingUserTest ? { evidence: {
				attempted: false,
				loaded: false,
				method: "unsupported",
		...
		if (input.retention === "persistent" && awaitingUserTest) hotReloadAttempt = { evidence: {
			attempted: false,
			loaded: false,
			method: "unsupported",
			reason: "Manual-runtime plugins are not activated inside the serving DSH process; restart is required before the real-client test."
		...
		try {
			...
			try {
				await this.store.put("installations", {
					...
					verification: {
						...verification,
						reason: input.retention === "temporary" ? `${verification.reason} Final receipt persistence failed; the owned temporary trial was removed.` : `${verification.reason} Final receipt persistence failed after runtime activation may have begun; recover by installationId and reconcile the exact live source.`
					}
				});
			} catch {}
...
function canonicalSignal(exec, signal) {
	if (exec?.signal && signal && exec.signal !== signal) throw new EvolutionError("invalid_input", "Built-in mutation received conflicting cancellation signals");
...
async function disableBuiltinMount(input) {
	const signal = canonicalSignal(void 0, input.signal);
	signal?.throwIfAborted();
...
async function enableBuiltinMount(input) {
	...
	if (wrote) {
		...
		signal?.throwIfAborted();
		if (journaledPreimage !== original) throw new EvolutionError("review_expired", "The target profile patch changed before the approved write; refusing a stale overwrite");
		rows.push({ insert: [{
			id: mountId,
			name: packageName
		}] });
	...
	if (!alreadyMounted(patchRows(live), mountId, packageName)) throw new EvolutionError("review_expired", "The exact built-in mount row changed during the composition check; recovery is required");
	return {
		packageName,
		version,
		mountId,
...
var PluginRemover = class {
	...
	async settleInterruptedRemoval(input) {
		...
		if (record.retention === "persistent" && builtin) try {
			if (builtin.wrote) await builtinMountPresent({
				dshHome: record.dshHome,
				targetProfile: record.targetProfile,
				mountId: builtin.mountId,
				packageName
...
async function revalidateReview(_deps, review, signal) {
	signal?.throwIfAborted();
...
var SourceManager = class SourceManager {
	...
	*/
	...
	async writeInitialLock(lockFile, body) {
		await writeFile(lockFile, body, {
			encoding: "utf8",
			flag: "wx"
		});
	}
	async releaseInitialLockIfExact(sourceId, lockToken, expectedBody) {
	...
	async acquireLockInternal(sourceId, workflowId, signal, workspaceCwd, allowVerifiedReentry = false) {
		...
		try {
			...
			if (!error || typeof error !== "object" || !("code" in error) || error.code !== "EEXIST") {
				await this.releaseInitialLockIfExact(sourceId, initialLock.lockToken, initialBody).catch(() => void 0);
				throw error;
			}
		}
		...
		try {
			...
			if (error && typeof error === "object" && "code" in error && error.code === "EEXIST") throw new EvolutionError("invalid_input", "Managed source lock recovery is already owned; refusing concurrent takeover", { sourceId });
			throw error;
		}
		try {
			signal?.throwIfAborted();
	...
	async acquireCompletionRecoveryOwner(sourceId, workflowId, proofHash, lockRaw, lockToken) {
		...
		if (orphanedTakeover !== void 0) {
			...
			if (!this.completionOwnerMatches(stale, proofHash, lockRaw, lockToken) || isLockHolderAlive(stale.pid)) return void 0;
		...
		if (!this.completionOwnerMatches(observedOwner, proofHash, lockRaw, lockToken) || isLockHolderAlive(observedOwner.pid)) return void 0;
		try {
	...
	*/
	async initializeCreateSource(input) {
		...
		try {
			...
			if (gitDirectoryExists) throw new EvolutionError("invalid_input", "Managed create source already exists; refusing to overwrite", { sourceId });
			input.signal?.throwIfAborted();
	...
	*/
	...
	async claimCompletedSourceForWorkflow(sourceId, workflowId, signal) {
		...
		try {
			...
			if (error && typeof error === "object" && "code" in error && error.code === "EEXIST") throw new EvolutionError("invalid_input", "Completed managed source was claimed concurrently");
			throw error;
		}
		if (await this.lockPublicationBarrierExists(sourceId)) {
...
async function adoptInstallation(deps, input, options = {}) {
	...
	options.signal?.throwIfAborted();
	const lineage = deriveInstallationLineage(installations);
	const tracked = trackedInstallation(installations, lineage, deps.config.dshHome, initialProfile, packageName, initialEvidence.evidence.dependencySpec);
	...
	options.signal?.throwIfAborted();
	const currentEvidence = currentProfile === initialProfile ? await exactProfileEvidence(deps, currentProfile, packageName, options.signal) : { status: "unreadable" };
	options.signal?.throwIfAborted();
	if (currentEvidence.status !== "found" || currentEvidence.evidence.dependencySpec !== initialEvidence.evidence.dependencySpec || currentEvidence.evidence.configuredBundle !== initialEvidence.evidence.configuredBundle) throw new EvolutionError("review_expired", "The exact profile evidence changed before the adoption claim; scan again");
	let claim;
	try {
	...
	options.signal?.throwIfAborted();
	let evidenceAfterClaim;
	try {
		evidenceAfterClaim = ownerAfterClaim === initialProfile ? await exactProfileEvidence(deps, initialProfile, packageName, options.signal) : { status: "unreadable" };
	} catch (error) {
		if (options.signal?.aborted) throw options.signal.reason;
		throw error;
	}
	...
	options.signal?.throwIfAborted();
	let evidenceAfterFinal;
	try {
		evidenceAfterFinal = ownerAfterFinal === initialProfile ? await exactProfileEvidence(deps, initialProfile, packageName, options.signal) : { status: "unreadable" };
	} catch (error) {
		if (options.signal?.aborted) throw options.signal.reason;
		throw error;
	}
...
function latestReleaseObject(stdout) {
	if (!stdout.trim()) throw new EvolutionError("github_unavailable", "GitHub returned empty latest release data");
	try {
...
async function listCapabilityVersions(deps, input, signal) {
	...
	signal?.throwIfAborted();
	const lineage = deriveInstallationLineage(listed.filter((record) => record.packageName === packageName));
...
async function rollbackInstallation(deps, input, exec) {
	...
	let liveSpec;
	if (deps.launcher.profileDependencySpec) try {
			...
			while (true) {
				...
				if (INTERRUPT_NODES.has(workflow.cursor)) {
					...
					if (workflow.cursor === "await_clarification") {
						...
						durableOutcomeCommitted = true;
						exec.signal?.throwIfAborted();
						const committed = await this.view(workflow, void 0, {}, exec.signal);
						exec.signal?.throwIfAborted();
						return committed;
					}
					if (!resolution && workflow.resolutionId) resolution = await this.awaitPreEffect(() => this.host.getResolution(workflow.resolutionId), exec.signal);
				...
				if (TERMINAL_NODES.has(workflow.cursor)) {
					...
					if (workflow.cursor === "recovery_required") {
						...
						durableOutcomeCommitted = true;
						exec.signal?.throwIfAborted();
						const committed = await this.view(workflow, resolution, {
							status: "parked",
							alreadyWaiting: true
						}, exec.signal);
						exec.signal?.throwIfAborted();
...
var WorkflowEngine = class extends WorkflowEngineRecovery {
	...
	async settleCommittedFinalDecisionFailure(workflow, resolution, exec) {
		workflow.cursor = "recovery_required";
		workflow.status = "interrupted";
		workflow.lastFailure = {
			stage: "workflow",
...
function validateStrictWorkflowRecord(stored) {
	...
	if (record.interrupt !== void 0) {
		...
		if (!INTERRUPT_KINDS.has(interrupt.kind) || !nonemptyString(interrupt.ownerSessionId) || !nonemptyString(interrupt.bootId) || typeof interrupt.snapshotDigest !== "string" || !/^[a-f0-9]{64}$/u.test(interrupt.snapshotDigest) || !Array.isArray(interrupt.options) || !plainObject(interrupt.facts)) throw new Error("workflow interrupt is invalid");
