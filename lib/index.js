...
var PluginInstaller = class {
	...
	async install(input, exec) {
		...
		await requestApproval$1(this.ctx, exec, `Install reviewed ${packageName} into profile ${input.targetProfile} (${input.retention}). Review: fit=${review.fit}, risk=${review.securityRisk}, compatibility=${review.compatibility.status}, lifecycleScripts=${scripts}, findings=${findings}.`, "plugin_install");
...
function marketplaceApprovalReason(requirement, profiles) {
	if (prefersChinese$1(requirement)) return `将把 DSH 插件市场 dsh-find-plugin 安装到 profile ${profiles.join("、")}。这是能力搜索用的基础设施，不是你要的那个能力。批准后会立刻安装，并尽量热加载到当前进程。`;
...
function copy(requirement, english, chinese) {
	return prefersChinese$1(requirement) ? chinese : english;
}
...
function adoptGithubCandidate(resolution, repositoryInput) {
	...
	const reason = prefersChinese(resolution.requirement) ? `已把 ${repository} 纳入审查。先看现成插件怎么写，不要直接自建。` : `Adopted ${repository} for review. Inspect the existing plugin before creating one.`;
	const authorization = {
	...
	return {
		...
		resolution: {
			...
			remoteCandidateSource: resolution.remoteCandidateSource === "marketplace-setup" || !resolution.remoteCandidateSource ? "github" : resolution.remoteCandidateSource,
			authorization,
...
function initialAuthorization(resolutionId, decision, remoteDiscoveryComplete, remoteCandidateSource) {
...
function authorizationForResolution(resolution, reviews) {
	...
	if (latestForCandidate.some((review) => review?.recommendation === "use")) return {
		state: "reuse_required",
		resolutionId: resolution.id,
		reason: marketplaceSetup ? "Install the reviewed DSH plugin marketplace, restart DSH, then call capability_resolve again." : "At least one reviewed candidate is a complete reusable fit."
	};
	...
	if (latestForCandidate.some((review) => review === void 0)) return {
		...
		resolutionId: resolution.id,
		reason: marketplaceSetup ? "Install the DSH plugin marketplace (awesome-dsh-plugin/dsh-find-plugin) first, or skip it only if the user declines. Direct GitHub search is not used as a fallback." : "Every discovered candidate must reach a terminal review before scratch development."
	};
	return {
		state: "scratch_ready",
		resolutionId: resolution.id,
		reason: marketplaceSetup ? "The user declined the plugin marketplace; one new Cordis Plugin may be defined." : "Every discovered candidate was reviewed and rejected; one new Cordis Plugin may be defined."
	};
}
