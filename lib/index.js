...
function nextStepForAuthorization(requirement, authorization) {
	...
	if (authorization.state === "confirmation_required") return zh ? "先在对话里讲清这次审查：匹配程度、风险、缺什么、主要发现。不要调用 ask_user。等用户回话后，再调用 capability_workflow_resume（只用 workflow_id 与 interrupt_id）。" : "Explain the review in chat (fit, risk, missing pieces, findings). Do not call ask_user. After the user replies, call capability_workflow_resume with only workflow_id and interrupt_id.";
...
var ExecutionGuard = class {
	...
	childDenial(name, exec) {
		...
		if (matchesSet(name, SHELL_TOOLS)) {
			...
			if (GIT_PUBLICATION_RE.test(command)) return "Managed source child session denies git push/tag/release and gh pr/release publication.";
		}
	}
};
//#endregion
...
async function discoverRemoteCandidates(options) {
	...
	if (options.ctx.tools.get("find_dsh_plugin", options.exec.agent)) {
		...
		const candidates = relevantRemoteCandidates(options.requirement, [...merged.values()]).sort((left, right) => right.stars - left.stars || left.repository.localeCompare(right.repository)).slice(0, options.config.maxCandidates);
...
*/
async function probeWorkspaceWriteSandbox(stack, expectedCwd) {
	if (!stack) throw new EvolutionError("invalid_input", "DSH sandbox stack is unavailable; modify/create cannot proceed", { reason: "missing_sandbox_stack" });
	if (!stack.filesystem) throw new EvolutionError("invalid_input", "DSH filesystem sandbox provider is unavailable; modify/create cannot proceed", { reason: "missing_filesystem_provider" });
	if (!stack.shell) throw new EvolutionError("invalid_input", "DSH shell sandbox provider is unavailable; modify/create cannot proceed", { reason: "missing_shell_provider" });
	const cwd = normalizePath(expectedCwd);
	const fsMode = stack.filesystem.mode;
	const shellMode = stack.shell.mode;
	if (fsMode !== "workspace-write") throw new EvolutionError("invalid_input", "Filesystem sandbox mode must be workspace-write for modify/create", {
		reason: "wrong_filesystem_mode",
		mode: fsMode
	});
	if (shellMode !== "workspace-write") throw new EvolutionError("invalid_input", "Shell sandbox mode must be workspace-write for modify/create", {
		reason: "wrong_shell_mode",
		mode: shellMode
	});
	const fsCwd = stack.filesystem.cwd ? normalizePath(stack.filesystem.cwd) : void 0;
	const shellCwd = stack.shell.cwd ? normalizePath(stack.shell.cwd) : void 0;
	if (!stack.filesystem.bindsManagedCwd && (!fsCwd || fsCwd !== cwd)) throw new EvolutionError("invalid_input", "Filesystem sandbox cwd is not bound to the managed source repository", {
		reason: "cwd_mismatch",
		expected: cwd,
		actual: fsCwd
	});
	if (!stack.shell.bindsManagedCwd && (!shellCwd || shellCwd !== cwd)) throw new EvolutionError("invalid_input", "Shell sandbox cwd is not bound to the managed source repository", {
		reason: "cwd_mismatch",
		expected: cwd,
		actual: shellCwd
	});
	...
	if (typeof stack.filesystem.assertContained === "function") {
		if (await stack.filesystem.assertContained(escapeCandidate)) throw new EvolutionError("invalid_input", "Filesystem sandbox failed containment probe (path escape accepted)", {
			reason: "containment_probe_failed",
			escapeCandidate
		});
	} else if (isPathInside$1(cwd, escapeCandidate)) throw new EvolutionError("invalid_input", "Filesystem sandbox containment probe is unsupported on this configuration", { reason: "unsupported_configuration" });
	if (typeof stack.shell.canWrite === "function") {
		if (await stack.shell.canWrite(escapeCandidate)) throw new EvolutionError("invalid_input", "Shell sandbox failed outside-cwd write probe", {
			reason: "shell_escape_probe_failed",
			escapeCandidate
		});
		if (!await stack.shell.canWrite(path.join(cwd, "probe.txt"))) throw new EvolutionError("invalid_input", "Shell sandbox rejected an in-cwd write required for workspace-write mode", { reason: "shell_incapable" });
	}
	return {
		ok: true,
		...
		note: process.platform === "win32" ? "Windows sandbox enforcement is integrity-oriented partial isolation; it does not claim confidentiality or network isolation." : "workspace-write sandbox probe passed for managed source modify/create."
	};
}
//#endregion
...
var SourceManager = class SourceManager {
	...
	async acquireLock(sourceId, workflowId, signal) {
		...
		if (!Boolean(existing.headCommit && existing.branch && head === existing.headCommit && branch === existing.branch && status === "")) throw new EvolutionError("invalid_input", "Managed source has a stale lock that failed revalidation", {
			sourceId,
			activeWorkflowId: existing.workflowId
		});
	...
	async buildNormalizedTgz(input) {
		...
		async function walk(directory, prefix = "") {
			...
			for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
				...
				if (info.isSymbolicLink()) throw new EvolutionError("unsafe_path", "Managed source contains a symlink; refusing to pack", { relative });
				if (info.isDirectory()) {
					await walk(absolute, relative);
					continue;
				}
				if (!info.isFile()) throw new EvolutionError("unsafe_path", "Managed source contains a special file; refusing to pack", { relative });
				if (!isPathInside(root, absolute)) throw new EvolutionError("unsafe_path", "Managed source file escaped repository root", { relative });
				files.push({
...
function resolveSandboxStack(ctx) {
	const direct = ctx.get("sandbox");
	if (direct && (direct.filesystem || direct.shell)) return direct;
	const filesystem = ctx.get("filesystem");
	const shell = ctx.get("shell");
	if (!filesystem && !shell) return void 0;
	return {
		...filesystem !== void 0 ? { filesystem } : {},
		...shell !== void 0 ? { shell } : {}
	};
}
...
var CapabilityEvolutionService = class {
	...
	async assertChildSandbox(sourceKey) {
	...
	async prepareModify(resolution, review, exec, workflow) {
		...
		return {
			resolution,
			path: (await this.sources.materializeReviewedGithub({
				review,
				workflowId: workflow.id,
				...exec.signal ? { signal: exec.signal } : {}
			})).path,
	...
	async prepareCreate(resolution, exec, workflow) {
		...
		return {
			resolution,
			path: (await this.sources.initializeCreateSource({
				resolutionId: resolution.id,
				workflowId: workflow.id,
				...exec.signal ? { signal: exec.signal } : {}
			})).path,
