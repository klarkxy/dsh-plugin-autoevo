...
const Config$1 = Schema.object({
	...
	maxCandidates: Schema.number().min(1).max(20).default(20),
...
var PluginInstaller = class {
	...
	async install(input, exec, binding) {
		...
		try {
			const actionPlan = this.preflightProfile && input.retention === "persistent" ? `Preflight the exact reviewed ${packageName} in an isolated headless profile, then install it into live profile ${input.targetProfile}` : input.retention === "temporary" ? `Install the exact reviewed ${packageName} into isolated temporary profile ${input.targetProfile}` : `Install the exact reviewed ${packageName} into profile ${input.targetProfile}`;
			await requestApproval$1(this.ctx, exec, `${riskPrefix}${actionPlan} (${input.retention}). Review: fit=${review.fit}, risk=${review.securityRisk}, compatibility=${review.compatibility.status}, lifecycleScripts=${scripts}, findings=${findings}.`, "capability_workflow_resume");
		} catch (error) {
			if (input.retention === "temporary") await this.removeOwnedDirectory(trialRoot, trialsRoot);
		...
		const hotReloadAttempt = input.retention === "persistent" && nonFailure ? await this.hotLoader({
			ctx: this.ctx,
			dshHome,
			profile: input.targetProfile,
			packageName,
...
function factsFor(view) {
	...
	if (state === "managed_work") return { operation: view.workflow.lastReviewId ? "modify" : "create" };
	return {
...
function modificationTask(resolution, review, blockers = modificationBlockers(review), focusedCorrection = false) {
	...
	return [
		...
		focusedCorrection ? "This is the single focused correction allowed after Host re-review. Investigate why the bounded targets below remain; do not mechanically assume a particular file or implementation." : "Use your own repository investigation and judgment to implement the smallest complete change.",
