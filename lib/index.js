import { c as EVOLUTION_PRESET_ID, d as EVOLUTION_PRESET_MANIFEST_FILENAME, f as OUTSIDE_EVOLUTION_MODE_DENIAL, l as EVOLUTION_PRESET_KNOWN_MANIFESTS, m as isEvolutionPresetManifest, o as EVOLUTION_MODE_OWNER, p as isEvolutionModeMarker, s as EVOLUTION_MODE_SERVICE_KEY, t as AUTOEVO_AUTONOMY_CONTRACT, u as EVOLUTION_PRESET_MANAGED_CONTENT_FILES } from "./evolution-mode2.js";
import { _ as hashObject, a as hostVerificationOverlay, c as selectInstallVerificationLayer, d as FORGED_RESUME_HOST_KEYS, f as POLICY_VERSION, g as classifyRuntimeSurface, h as VERIFICATION_STATUSES, i as hostLayerSuccess, l as verificationChildEnv, m as VERIFICATION_LAYER_KINDS, n as declaredVerificationFixturesFromPackage, o as inspectLoadedToolSafety, p as TOOL_NAMES, r as fixtureDigestFor, s as sanitizeHostVerificationEvidence, u as BRIDGE_EXECUTION_TOOLS, v as sha256 } from "./host-verification-driver.js";
...
var CreationGuard = class {
	...
	*/
	...
	protocolDenial(exec) {
		...
		if (exec.name === FIND_PLUGIN_TOOL$2 && exec.parent === void 0) {
...
function rejectCodePreset(actual) {
	if (actual === "code") throw creatorUnavailable("Managed construction requires the official Creator cordis preset; the code preset is not permitted and there is no fallback", {
		actual,
...
function defaultAcceptanceTargets(operation) {
	if (operation === "create") return ["Host local re-review must produce an installable managed snapshot", "Do not install, publish, or claim success from the child session"];
...
function assertCreatorReceipt(receipt, preflight) {
	if (!receipt) throw creatorUnavailable("Managed child did not return a verified Creator foundation receipt");
	rejectCodePreset(receipt.presetId);
	if (receipt.contractVersion !== 1) throw creatorUnavailable("Managed child Creator foundation receipt contractVersion mismatch", {
		expected: 1,
		actual: receipt.contractVersion
	});
	assertNotCodePresetId(receipt.presetId);
	if (receipt.compositionSha256 !== preflight.compositionSha256) throw creatorUnavailable("Managed child composition SHA-256 does not match Creator preflight");
	if (receipt.requiredToolCatalogDigest !== preflight.requiredToolCatalogDigest) throw creatorUnavailable("Managed child required tool catalog digest does not match Creator preflight");
	if (typeof receipt.childSessionId !== "string" || receipt.childSessionId.trim().length === 0) throw creatorUnavailable("Managed child Creator foundation receipt is missing the child session identity");
	return receipt;
}
...
function assertNotCodePresetId(id) {
	rejectCodePreset(id);
	if (id !== "cordis") throw creatorUnavailable("Managed construction requires the official Creator cordis preset; no other preset and no fallback is permitted", {
		actual: id,
		expected: CREATOR_PRESET_ID
	});
}
async function rosterItems(agentPresets) {
	if (typeof agentPresets.list !== "function") return void 0;
	const listed = await agentPresets.list();
	if (Array.isArray(listed)) return listed;
	if (isRecord$2(listed) && Array.isArray(listed.presets)) return listed.presets;
	return [];
}
...
function assertRequiredCreatorCatalog(catalog, platform = process.platform) {
	...
	for (const skill of OFFICIAL_CREATOR_SKILLS) if (!actualSkills.has(skill)) missing.push(skill);
	if (missing.length > 0) throw creatorUnavailable("Official Creator cordis catalog is missing required tools or skills", { missing });
}
...
async function assertChildCreatorCatalog(agentCtx, childScope, preflight, composedPreset, mountedComposition) {
	rejectCodePreset(composedPreset);
	if (composedPreset !== "cordis") throw creatorUnavailable("Managed child did not compose the official Creator cordis preset; code and fallback presets are not permitted", {
		actual: composedPreset,
		expected: CREATOR_PRESET_ID
	});
	if (compositionSha256(mountedComposition) !== preflight.compositionSha256) throw creatorUnavailable("Managed child mounted Creator composition does not match the preflight SHA-256");
	const catalog = await collectCreatorCatalog(agentCtx, childScope);
	assertRequiredCreatorCatalog(catalog);
	if (requiredToolCatalogDigest(requiredCreatorCatalog()) !== preflight.requiredToolCatalogDigest) throw creatorUnavailable("Managed child required tool catalog digest does not match Creator preflight");
	return catalog;
}
async function preflightCreatorFoundation(ctx, input = {}) {
	const agentPresets = serviceFrom(ctx, "agentPresets");
	if (!agentPresets) throw creatorUnavailable("DSH agent preset service is required to resolve the official Creator cordis preset");
	const missingRuntime = [
		...
		"tools",
		"skills"
	].filter((name) => serviceFrom(ctx, name) === void 0);
	if (input.parentCtx && serviceFrom(input.parentCtx, "agents") === void 0) missingRuntime.push("parent agents");
	if (missingRuntime.length > 0) throw creatorUnavailable("Managed Creator child runtime prerequisites are unavailable", { missing: missingRuntime });
	...
	if (roster) {
		...
		if (!entry) throw creatorUnavailable("Official Creator cordis preset is missing from the DSH agent preset roster", { sawCodePreset: Boolean(codeEntry) });
		if (entry.broken) throw creatorUnavailable("Official Creator cordis preset composition is broken and cannot be mounted");
	}
	if (typeof agentPresets.resolve !== "function") throw creatorUnavailable("DSH agent preset service cannot resolve the official Creator cordis preset");
	let resolved;
	try {
		resolved = await agentPresets.resolve(CREATOR_PRESET_ID);
	} catch (error) {
		throw creatorUnavailable("Official Creator cordis preset could not be resolved", { cause: error instanceof Error ? error.message : String(error) });
	}
	if (!resolved || resolved.id !== "cordis" || resolved.trust !== "system") throw creatorUnavailable("Resolved cordis preset is not the official system Creator preset");
	if (resolved.broken) throw creatorUnavailable("Resolved official Creator cordis preset is broken and cannot be mounted");
	if (typeof agentPresets.read !== "function") throw creatorUnavailable("DSH agent preset service cannot read the official Creator cordis composition");
	let composition;
	try {
		composition = await agentPresets.read(CREATOR_PRESET_ID);
	} catch (error) {
		throw creatorUnavailable("Official Creator cordis composition could not be read", { cause: error instanceof Error ? error.message : String(error) });
	}
	if (typeof composition !== "string" || !compositionLooksMountable(composition)) throw creatorUnavailable("Official Creator cordis composition is missing, empty, or not a mountable Creator composition");
	if (typeof agentPresets.standingKeyFor !== "function") throw creatorUnavailable("Official Creator cordis preset cannot be mount-validated; standing scope is unavailable");
	let standingScope;
	try {
		standingScope = await agentPresets.standingKeyFor(CREATOR_PRESET_ID);
	} catch (error) {
		throw creatorUnavailable("Official Creator cordis composition is unmountable", { cause: error instanceof Error ? error.message : String(error) });
	}
	if (standingScope === void 0 || standingScope === null || standingScope === false) throw creatorUnavailable("Official Creator cordis preset standing scope was not obtained");
...
var ExecutionGuard = class {
	...
	parentDenial(name, exec) {
		...
		if (isNewCordisDefinition(exec)) return "AutoEvo parent session denies cordis_define(kind:new); create-new continues only in a Host-launched managed git source child.";
...
function buildManifest(files, templateVersion = "12") {
	const ordered = {};
...
async function materializeEvolutionPresetLocked(options, paths, presetsRoot, targetDir, renamePath) {
	const templateVersion = options.templateVersion ?? "12";
...
const PROFILE_NAME$1 = /^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u;
...
function explicitProfileSelection(argv) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (!PROFILE_NAME$1.test(profile)) return { state: "invalid" };
...
async function resolveCurrentProfileOwner(input) {
	...
	const owners = [];
	for (const entry of entries) {
		if (!PROFILE_NAME$1.test(entry.name)) continue;
...
async function resolveProfilePluginCapabilities(input) {
	if (!PROFILE_NAME$1.test(input.profile)) return [];
...
async function resolveLocalCapabilities(ctx, requirement, exec, options = {}) {
	...
	const useful = candidates.some((candidate) => candidate.confidence >= .62 && candidate.fit === "full");
	return {
		cwd,
...
async function discoverRemoteCandidates(options) {
	...
	if (options.ctx.tools.get("find_dsh_plugin", options.exec.agent)) {
		const planned = options.queries ? [...new Set(options.queries.map((query) => boundedText(query, 120)).filter((query) => query.length >= 2))].slice(0, 5) : marketplaceSearchQueries(options.requirement);
...
var PluginInstaller = class {
	...
	async install(input, exec, binding) {
		...
		try {
			await requestApproval$1(this.ctx, exec, installApprovalReason({
				requirement: review.requirement,
				packageName,
...
function marketplaceApprovalReason(requirement, profiles) {
	return copy(requirement, `Install the DSH plugin marketplace dsh-find-plugin into profile ${profiles.join(", ")}. This is search infrastructure, not the requested capability. After approval AutoEvo installs it and tries to hot-load it into this process.`, `将把 DSH 插件市场 dsh-find-plugin 安装到 profile ${profiles.join("、")}。这是能力搜索用的基础设施，不是你要的那个能力。批准后会立刻安装，并尽量热加载到当前进程。`);
}
...
async function installMarketplace(options) {
	...
	if (!PROFILE_NAME.test(options.profile) || !await options.launcher.hasProfileDependency(options.config.dshHome, options.profile, "dsh-plugin-autoevo")) return {
		status: "no_profile",
		profiles: [],
		reason: copy(requirement, "The live DSH profile that owns this process does not record AutoEvo; refusing to select another profile for marketplace installation.", "当前进程所属的 DSH profile 没有记录 AutoEvo；拒绝改选其它 profile 安装插件市场。")
	};
	const pending = [];
	...
	async run(request) {
		...
		const handle = await parentAgents.create({
			...
			setup: async (agentCtx) => {
				...
				if (mounted.id !== "cordis" || mounted.trust !== "system" || composed !== "cordis") throw new EvolutionError("invalid_input", "Managed child did not mount the official Creator cordis preset; the code preset and any fallback are not permitted", {
					mounted: mounted.id,
					trust: mounted.trust,
					composed
				});
...
const MODEL_CONTROL_NODES = /* @__PURE__ */ new Set(["await_discovery"]);
...
function isInterruptKind(value) {
	return value === "await_selection" || value === "await_confirmation" || value === "await_modify_work" || value === "await_recovery";
}
...
function optionsFor(kind, resolution, reviews = [], workflow, installProfiles = []) {
	...
	if (kind === "await_confirmation") {
		...
		const usableIds = reviews.filter((item) => isDirectlyUsableReview(item, workflow) && !consumed.some((attempt) => sameVerificationAttempt(attempt, item))).map(candidateIdFor).filter((id) => Boolean(id));
...
function factsFor(view) {
	...
	if (state === "waiting_candidate_selection") return { sealed_candidates: candidateEvidence(view.workflow.candidateSnapshot ?? [], view.resolution) };
...
async function executeDiscoverRemote(ctx) {
	...
	if (nextUnseenRemote(resolution, ctx.workflow) || hasFullLocal || !resolution.remoteDiscoveryComplete) return {
		kind: "next",
...
async function executePrepareModify(ctx) {
	...
	if (modificationAttemptsExhausted(ctx.workflow.modificationOutcome)) {
		ctx.workflow.lastFailure = {
			...
			message: ctx.workflow.modificationOutcome?.introducedBlockers.length ? "Host re-review found new blocking modification targets; another child will not be started." : "Modification already used its two Host-bounded attempts. Diagnose or choose a different reviewed action; Host will not start another child.",
			retryable: false
		};
		return {
			kind: "next",
...
function selectionNavigationFromMisplacedDecision(workflow, decision) {
	if (workflow.cursor !== "await_selection" || !decision) return void 0;
	if (decision.action === "stop") return { kind: "stop" };
	if (decision.action !== "use_this" && decision.action !== "modify_this") return void 0;
	const candidateId = decision.candidateId;
	if (!candidateId) return void 0;
	const item = (workflow.candidateSnapshot ?? []).find((entry) => entry.id === candidateId);
	if (decision.action === "use_this" && item?.kind === "local" && item.fit === "full") return {
		kind: "reuse_local",
		candidateIds: [candidateId]
	};
	if (!item || item.kind !== "remote") return void 0;
	return {
...
var WorkflowEngine = class {
	...
	async startFresh(requirement, normalized, sessionId, cwd, exec, recoveredFromWorkflowId, workflowId = newWorkflowId(requirement)) {
	...
	async recover(input, exec) {
		...
		return await this.startFresh(restart.requirement, restart.normalized, restart.sessionId, restart.cwd, exec, restart.oldWorkflowId, restart.workflowId);
	}
	...
	async resume(input, exec) {
		return await this.withLock(input.workflowId, async () => {
			...
			const coercedNavigation = !input.navigation ? selectionNavigationFromMisplacedDecision(workflow, input.decision) : void 0;
			if (coercedNavigation) try {
	...
	async findReusableWorkflow(sessionId, cwd, requirementNormalized) {
		return (await this.store.listWorkflows()).filter((item) => isUnfinished(item.status) && item.ownerSessionId === sessionId && item.cwd === cwd && item.requirementNormalized === requirementNormalized && item.policyVersion === "8").sort((left, right) => right.updatedAt.localeCompare(left.updatedAt))[0];
	}
...
function childCheckEvidence(taskResult) {
	const report = taskResult.replace(/\s*AUTOEVO_CHILD_COMPLETED\s*$/u, "");
	if (reportsGenuineTestFailure(report)) return {
	...
	if (reportsUnavailableLocalToolchain(report)) return {
		source: "child_reported",
		status: "unavailable",
	...
	if (/skipped (?:the )?(?:test|tests|test run)|tests? (?:were )?not run|未运行测试|跳过测试/iu.test(report)) return {
		source: "child_reported",
		status: "skipped",
		summary: "The managed child reported that tests were skipped."
	};
	if (/(?:tests?|test run).{0,60}(?:passed|successful)|测试.{0,40}通过/iu.test(report)) return {
		source: "child_reported",
		status: "passed",
		summary: "The managed child reported that tests passed; Host did not independently observe the command result."
	};
	if (new RegExp(TEST_FAILURE, "iu").test(report)) return {
		source: "child_reported",
		status: "failed",
		summary: "The managed child reported that tests failed; Host did not independently observe the command result."
	};
...
function modificationWorkOrder(resolution, review, cwd, blockers = modificationBlockers(review), focusedCorrection = false) {
...
var CapabilityEvolutionService = class {
	...
	semanticVerifier;
	creatorFoundation;
	constructor(ctx, config, runner, store, creationGuard, managedChild, semanticReviewer, semanticVerifier, creatorFoundation) {
		this.ctx = ctx;
		this.config = config;
		this.runner = runner;
		this.store = store;
		...
		}, void 0, this.semanticVerifier, "headless", () => this.currentProfileOwner());
	...
	async ensureMarket(resolution, exec) {
		...
		const authorization = {
			state: "market_required",
			resolutionId: resolution.id,
			reason: prefersChinese(resolution.requirement) ? "市场插件已写入 profile，但当前进程热加载失败。请重启 DSH，再调用 capability_workflow。" : "The marketplace plugin is a profile dependency, but this process could not hot-load it. Restart DSH, then call capability_workflow again."
		};
		const next = withNextStep({
			...resolution,
			authorization,
	...
	async prepareModify(resolution, review, exec, workflow) {
		...
		try {
			...
			for (let attempt = 1; attempt <= 2; attempt += 1) {
				const operation = attempt === 1 ? "modify" : "correct";
				activeOperation = operation;
				const workOrder = modificationWorkOrder(resolution, review, receipt.path, correctionTargets, attempt === 2);
				...
				const committed = await this.sources.finalizeChildCommit({
					...
					workflowId: workflow.id,
					reviewId: currentReview.id,
					message: attempt === 1 ? `fix: satisfy AutoEvo workflow ${workflow.id}` : `fix: complete AutoEvo workflow ${workflow.id}`,
					...exec.signal ? { signal: exec.signal } : {}
				});
				...
				workflow.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
				await this.store.put("workflows", workflow);
				if (!acceptance.canCorrect) return {
					resolution: currentResolution,
...
function presentResumePendingCard(args) {
	...
	if (action === "modify_this") return genericPendingCard(args, "AutoEvo is improving and checking the plugin; this may take several minutes", "AutoEvo 正在改进并检查插件，可能需要几分钟", "edit");
	if (action === "create_new") return genericPendingCard(args, "AutoEvo is creating a new plugin; this may take several minutes", "AutoEvo 正在创建新插件，可能需要几分钟", "edit");
	...
	if (navKind === "reuse_local") return genericPendingCard(args, "Checking already-installed local capabilities", "正在检查已安装的本地能力", "read");
	...
	return [
		defineTool({
			...
			async execute(args, exec) {
				return compactAgentView(await service.start(args.requirement, exec));
			}
		}),
		defineTool({
