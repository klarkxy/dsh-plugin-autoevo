...
const INDEX_LIST_RE = new RegExp(`(?:${SELECT_PREFIX})((?:[一二两三四五六七八九十1-9])(?:\\s*[和、,，]\\s*(?:[一二两三四五六七八九十1-9]))+)`, "gu");
...
function addIndexToken(found, token, remotes) {
...
function mentionedRepositories(message, remotes, previouslySelected = []) {
	...
	if (found.size === 0 && (THIS_RE.test(message) || INSTALL_RE.test(message))) {
		if (previouslySelected.length > 0) for (const repository of previouslySelected) found.add(repository);
		else if (remotes.length === 1) found.add(remotes[0].repository);
	}
	return [...found];
}
function wantsUseThis(message) {
...
function inferAction(message, remotes, locals, phase, previouslySelected) {
	...
	if (phase === "gate2" && wantsUseThis(message)) return {
		action: "use_this",
		selectedRepositories: selected
	};
...
function resolveDecision(input) {
	...
	const inferred = inferAction(userMessage, input.remotes, input.locals, input.phase, input.previouslySelected ?? []);
...
var CreationGuard = class {
	...
	protocolDenial(exec) {
		...
		const waiting = state?.authorization?.state === "selection_required" || state?.authorization?.state === "confirmation_required";
		if (exec.name === FIND_PLUGIN_TOOL$2 && exec.parent === void 0) return "Use the shortlist from capability_resolve. Call capability_decide or plugin_review; do not search again.";
		if (exec.name === WEB_SEARCH_TOOL && waiting) return "Discovery is finished. Call capability_decide with the user's latest message.";
		if (SHELL_TOOLS.has(exec.name) && state?.authorization && isDshPluginAddCommand(shellCommandText(exec.arguments))) return "Install only via plugin_install after review.";
	}
...
var CapabilityEvolutionService = class {
	...
	async ensureInspectFromLastMessage(resolution, repository, exec) {
		if ((resolution.selectedRepositories ?? []).map((item) => item.toLowerCase()).includes(repository.toLowerCase())) return resolution;
	...
	async ensureUseThisFromLastMessage(review, exec) {
		const resolution = await this.store.getResolution(review.resolutionId);
		try {
			this.creationGuard.assertInstallAuthorized(exec.agent, review, resolution);
			return;
