...
function isDirectlyUsableReview(review, workflow) {
	if (review.policyVersion !== "9") return false;
...
function assertDirectUseAllowed(review, workflow) {
	if (review.policyVersion !== "9") throw new EvolutionError("review_rejected", "This review predates the current policy and cannot authorize installation", {
		reviewId: review.id,
...
function authorizationForResolution(resolution, reviews = []) {
	if (resolution.schemaVersion !== 2 || resolution.policyVersion !== "9" || !resolution.authorization) return {
		state: "selection_required",
...
function lifecycleStateFor(workflow, extras = {}) {
	if (workflow.policyVersion !== "9" || workflow.lastFailure?.code === "policy_restart_required") return "interrupted";
...
var WorkflowEngineCore = class {
	...
	assertOwner(workflow, exec) {
		...
		if (workflow.policyVersion !== "9") throw new EvolutionError("invalid_input", "Workflow predates the current policy and cannot be controlled");
	}
