...
function denialReason(authorization) {
	...
	if (authorization.state === "market_required") return `${prefix}: install the DSH plugin marketplace first, restart DSH, then call capability_resolve again. ${authorization.reason}`;
...
function matchConfidence(requirement, name, description) {
	...
	for (const anchor of anchors) {
		...
		for (const alias of anchor.aliases) {
			...
			if (normalizedDescription.includes(alias) && !isNameDropMention(normalizedDescription, alias)) strength = Math.max(strength, .58);
		}
		if (anchor.generic) {
...
const MARKETPLACE_SETUP_CANDIDATE = {
