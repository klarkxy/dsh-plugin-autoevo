...
function resolveInstallFromDecision(interrupt, decision, requirement, verificationLayer) {
	...
	if (evolutionTarget && decision.retention === "temporary") throw new EvolutionError("invalid_input", "Replacing an installed plugin requires persistent retention");
...
function evolutionTargetFromProfile(input) {
	...
	const owned = input.installation && !input.installation.removed && !input.installation.supersededByInstallationId;
	return {
...
function isNamedTarget(candidate, intent) {
	...
	return candidate.name.toLowerCase() === wanted || candidate.profileEvidence?.packageName.toLowerCase() === wanted;
}
...
const WORKFLOW_OPTIONS = {
	...
	review_existing: {
		id: "review_existing",
		labelEn: "Review the installed plugin source",
		labelZh: "审查已安装插件的已知来源",
		placement: "primary"
	},
	search_more: {
		id: "search_more",
...
var CapabilityEvolutionService = class {
	...
	async reviewExisting(resolution, target, exec, workflow) {
		...
		if (!expected || expected.repository.toLowerCase() !== target.repository.toLowerCase() || expected.commit !== target.commit || expected.specDigest !== target.specDigest || expected.packageName !== target.packageName || expected.profile !== target.profile) throw new EvolutionError("invalid_input", "Installed-source review must use the frozen Host evolution target");
	...
	async prepareModify(resolution, review, exec, workflow) {
		...
		if (sourceKey) receipt = await this.sources.resumeWorkflowSource(sourceKey, workflow.id, exec.signal);
			...
			if (completed && completed.repository?.toLowerCase() === review.sourceSnapshot.repository.toLowerCase() && completed.headCommit.toLowerCase() === review.sourceSnapshot.commit.toLowerCase()) receipt = await this.sources.claimCompletedSourceForWorkflow(sourceKey, workflow.id, exec.signal);
...
function presentResumePendingCard(args) {
	...
	if (navKind === "review_existing") return genericPendingCard(args, "Reviewing the installed plugin's known source; this is not a modification", "正在审查已安装插件的已知来源，这不是修改", "read");
