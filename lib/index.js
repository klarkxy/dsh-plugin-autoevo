...
import { _ as sha256, a as inspectLoadedToolSafety, c as verificationChildEnv, d as POLICY_VERSION, f as TOOL_NAMES, g as hashObject, h as classifyRuntimeSurface, i as hostVerificationOverlay, l as BRIDGE_EXECUTION_TOOLS, m as VERIFICATION_STATUSES, n as declaredVerificationFixturesFromPackage, o as sanitizeHostVerificationEvidence, p as VERIFICATION_LAYER_KINDS, r as hostLayerSuccess, s as selectInstallVerificationLayer, u as FORGED_RESUME_HOST_KEYS } from "./host-verification-driver.js";
...
function activeProfileFromArgv(argv) {
	const profiles = [];
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (!PROFILE_NAME$1.test(profile)) return void 0;
		profiles.push(profile);
	}
	if (profiles.length === 0) return void 0;
	return profiles.every((profile) => profile === profiles[0]) ? profiles[0] : void 0;
}
...
var PluginInstaller = class {
	...
	async install(input, exec, binding) {
		...
		const loaded = sourceMatched && (verified || activated || awaitingUserTest || verification.attempted && verification.exitCode === 0);
...
function userFacingMeaning(action, completedCleanup = false) {
	return {
		capability_workflow_refine: "继续补充只读发现证据",
		capability_workflow_present: "形成最终候选短名单",
		capability_workflow_recover: completedCleanup ? "用户明确要求清理并从头开始时，清理本次安装并从原始需求重新发现" : "清理本次工作流拥有的安装，并从原始需求重新发现",
...
function genericPendingCard(title, kind) {
	return {
		card: "generic",
		title,
		kind
...
function presentResumePendingCard(args) {
	...
	if (action === "modify_this") return genericPendingCard("AutoEvo is improving and checking the plugin; this may take several minutes", "edit");
	if (action === "create_new") return genericPendingCard("AutoEvo is creating a new plugin; this may take several minutes", "edit");
	if (action === "use_this") return genericPendingCard("Installing and verifying the reviewed plugin; this may take several minutes", "execute");
	if (action === "stop" || navKind === "stop") return genericPendingCard("Stopping this capability request", "other");
	if (navKind === "review_candidates") return genericPendingCard("Reviewing selected plugin candidates", "read");
	if (navKind === "search_more") return genericPendingCard("Searching for more plugin candidates", "search");
	if (navKind === "reuse_local") return genericPendingCard("Checking already-installed local capabilities", "read");
