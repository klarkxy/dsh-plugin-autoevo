...
function denialReason(authorization) {
	...
	if (authorization.state === "create_authorized") return `${prefix}: create-new continues as in-session work on a Host-managed git source; cordis_define(kind:new) is not permitted.`;
...
function assertRequiredCreatorCatalog(catalog, platform = process.platform) {
	...
	if (missing.length > 0) throw creatorUnavailable("Capability Evolution parent catalog is missing required construction tools", { missing });
}
...
var ExecutionGuard = class {
	...
	parentDenial(name, exec) {
		...
		if (matchesSet(name, DELEGATION_TOOLS)) return "AutoEvo parent session denies subagent, workflow, and ralph delegation; keep work visible in this session.";
		if (isNewCordisDefinition(exec)) return "AutoEvo parent session denies cordis_define(kind:new); create-new continues as in-session work on a Host-managed git source.";
...
function buildManifest(files, templateVersion = "13") {
	const ordered = {};
...
async function materializeEvolutionPresetLocked(options, paths, presetsRoot, targetDir, renamePath) {
	const templateVersion = options.templateVersion ?? "13";
...
var SourceManager = class SourceManager {
	...
	async receiptForManagedPath(candidate) {
		...
		if (!isPathInside$1(this.sourceRoot, resolved) || path.dirname(resolved) !== path.resolve(this.sourceRoot)) return void 0;
	...
	async disabledHooksPath() {
		...
		if (!isPathInside$1(this.sourceRoot, resolved)) throw new EvolutionError("unsafe_path", "Host disabled-hooks directory escaped sourceDir");
		return resolved;
	}
	...
	async assertPathContainment(sourceId) {
		...
		if (!isPathInside$1(this.sourceRoot, resolved)) throw new EvolutionError("unsafe_path", "Managed source realpath escaped sourceDir", {
			sourceId,
			resolved
		});
		return resolved;
...
var CapabilityEvolutionService = class {
	...
	constructor(ctx, config, runner, store, creationGuard, _managedChild, _semanticReviewer, _semanticVerifier, creatorFoundation) {
		...
		}, void 0, void 0, "headless", () => this.currentProfileOwner());
