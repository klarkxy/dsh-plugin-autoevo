...
var SourceManager = class SourceManager {
	...
	*/
	...
	async disabledHooksPath() {
		...
		if (!isPathInside(resolveStateRoot(this.config), resolved)) throw new EvolutionError("unsafe_path", "Host disabled-hooks directory escaped AutoEvo stateDir");
		return resolved;
	}
...
async function prepareManagedModification(deps, resolution, review, exec, workflow) {
	...
	if (sourceKey) {
		...
		const completed = await deps.sources.inspectCompletedSource(sourceKey, exec.signal);
		if (Boolean(completed && completed.repository?.toLowerCase() === review.sourceSnapshot.repository.toLowerCase() && completed.headCommit.toLowerCase() === review.sourceSnapshot.commit.toLowerCase() && deps.sources.pathUnderSourceRoot(completed.path, resolution.cwd)) && completed) receipt = await deps.sources.claimCompletedSourceForWorkflow(sourceKey, workflow.id, exec.signal);
