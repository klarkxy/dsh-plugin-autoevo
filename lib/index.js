...
var CommunityQualityService = class {
	...
	async screen(candidates, signal) {
		...
		try {
			...
			if (!value || typeof value !== "object" || !Array.isArray(value.assessments)) throw new TypeError("invalid community quality response");
	...
	async flushPending(limit = 20) {
		...
		for (const entry of entries.filter((name) => /^quality_[a-f0-9]{32}\.json$/u.test(name)).sort().slice(0, limit)) try {
	...
	async requestJson(relative, body, signal) {
...
3. After discovery, present each candidate in chat: repository, what it does, why it matched, stars, and community quality when present. Broken/junk filtering is optional and never replaces the exact-commit security review. Do not call ask_user. Wait for the user's reply, then call capability_decide. Do not review unselected repositories. Empty search is not permission to create.
