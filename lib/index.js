import { a as EVOLUTION_MODE_SERVICE_KEY, c as EVOLUTION_PRESET_MANAGED_CONTENT_FILES, f as isEvolutionModeMarker, i as EVOLUTION_MODE_OWNER, l as EVOLUTION_PRESET_MANIFEST_FILENAME, n as isWorkflowSkill, p as isEvolutionPresetManifest, s as EVOLUTION_PRESET_ID, u as OUTSIDE_EVOLUTION_MODE_DENIAL } from "./creator-skill.js";
...
async function verifyPristine(targetDir, manifest) {
	...
	for (const name of children) {
		...
		if (!(await stat(childPath)).isFile()) return {
			ok: false,
...
async function readInstalledManifest(targetDir) {
	...
	try {
		...
		return isEvolutionPresetManifest(raw) ? raw : void 0;
	} catch {
		return;
	}
}
...
async function materializeEvolutionPreset(options) {
	...
	if (!await pathExists(paths.targetDir)) {
		await mkdir(paths.presetsRoot, { recursive: true });
		assertContained(paths.dshHome, paths.presetsRoot, "presets root create");
		const stagingDir = assertContained(paths.presetsRoot, path.join(paths.presetsRoot, `.${EVOLUTION_PRESET_ID}.staging-${randomSuffix()}`), "staging");
		try {
			...
			await cleanupOwnedTree(stagingDir, paths.presetsRoot).catch(() => void 0);
			throw error;
		}
	}
	...
	const sameHashes = EVOLUTION_PRESET_MANAGED_CONTENT_FILES.every((relative) => installedManifest.files[relative] === desiredManifest.files[relative]);
	const sameFileSet = Object.keys(installedManifest.files).length === Object.keys(desiredManifest.files).length && Object.keys(desiredManifest.files).every((key) => key in installedManifest.files);
	if (sameVersion && sameHashes && sameFileSet) return {
		status: "noop",
		targetDir: paths.targetDir,
		reason: "template version and managed file hashes already match",
		templateVersion
	};
	const stagingDir = assertContained(paths.presetsRoot, path.join(paths.presetsRoot, `.${EVOLUTION_PRESET_ID}.staging-${randomSuffix()}`), "upgrade staging");
	const backupDir = assertContained(paths.presetsRoot, path.join(paths.presetsRoot, `.${EVOLUTION_PRESET_ID}.backup-${randomSuffix()}`), "upgrade backup");
	try {
		await writeStagedPreset(stagingDir, contentFiles, desiredManifest);
		...
		await cleanupOwnedTree(backupDir, paths.presetsRoot).catch(() => void 0);
		...
		await cleanupOwnedTree(stagingDir, paths.presetsRoot).catch(() => void 0);
		if (await pathExists(backupDir) && !await pathExists(paths.targetDir)) await renamePath(backupDir, paths.targetDir).catch(() => void 0);
		else if (await pathExists(backupDir) && await pathExists(paths.targetDir)) await cleanupOwnedTree(backupDir, paths.presetsRoot).catch(() => void 0);
		throw error;
	}
}
//#endregion
...
function asSearchResponse(stdout) {
	...
	try {
		...
		throw new EvolutionError("github_unavailable", `GitHub returned malformed repository search data (${Buffer.byteLength(cleaned)} bytes): ${preview}`, {
