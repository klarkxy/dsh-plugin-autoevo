...
function resolveDecisionTarget(decision, interrupt) {
	...
	if (decision.retention !== void 0) throw new EvolutionError("invalid_input", "Authorization decisions do not accept retention under Policy V10");
...
function validateReviewerRunInput(input) {
	...
	if (!DIGEST_RE$1.test(input.snapshotDigest) || !DIGEST_RE$1.test(input.candidateDigest)) throw new EvolutionError("invalid_input", "snapshotDigest and candidateDigest must be 64-character hex digests");
...
function rejectForgedReviewerSubmit(args) {
	rejectForgedSubmit(args, FORGED_REVIEWER_SUBMIT_KEYS, SUBMIT_KEYS$1, REVIEWER_SUBMIT_TOOL);
}
...
function scanContent(files, manifest) {
	...
	for (const file of files) {
		...
		else if (text.includes("\0")) findings.push(finding("hidden_instructions", "block", file.path, "contains NUL bytes in text", fileHash));
...
function isDirectlyUsableReview(review, workflow) {
	if (review.policyVersion !== "10") return false;
...
function assertDirectUseAllowed(review, workflow) {
	if (review.policyVersion !== "10") throw new EvolutionError("review_rejected", "This review predates the current policy and cannot authorize installation", {
		reviewId: review.id,
...
var ExecutionGuard = class {
	...
	parentDenial(name, exec) {
		...
		if (matchesSet(name, PARENT_DENIED_CORDIS_TOOLS) || normalizedBridgeTarget && matchesSet(normalizedBridgeTarget, PARENT_DENIED_CORDIS_TOOLS) || isNewCordisDefinition(exec)) return "Capability Evolution Policy V10 denies Cordis live mutation in the parent session; use the Search-first workflow.";
		if (matchesSet(name, SEARCH_BYPASS_TOOLS) || normalizedBridgeTarget && matchesSet(normalizedBridgeTarget, SEARCH_BYPASS_TOOLS)) return "Capability Evolution Policy V10 denies direct or nested find_dsh_plugin; start or resume capability_workflow.";
		if (matchesSet(name, SKILL_TOOLS)) {
			const target = skillTargetFromArguments(exec.arguments);
			if (!target || target === "cordis-plugin-development") return "Capability Evolution Policy V10 denies loading cordis-plugin-development in the parent session.";
		}
		if (matchesSet(name, DELEGATION_TOOLS) || normalizedBridgeTarget && matchesSet(normalizedBridgeTarget, DELEGATION_TOOLS)) return "Capability Evolution Policy V10 denies ordinary model, subagent, agent, and workflow delegation in the parent session.";
		...
		if (matchesSet(name, SHELL_TOOLS)) {
			...
			if (!isSafeShellCommand(command, SAFE_PARENT_SHELL_RE)) return "Capability Evolution Policy V10 permits only allowlisted read-only shell inspection commands in the parent session.";
		}
	}
...
function validateVerifierRunInput(input) {
	...
	if (!DIGEST_RE$1.test(input.evidenceDigest)) throw new EvolutionError("invalid_input", "evidenceDigest must be a 64-character hex digest");
	assertTimeoutWithinBound(input.timeoutMs);
...
function pnpmBlockedGitPackages(stderr) {
	if (!stderr.includes(PNPM_GIT_PREPARE_ERROR)) return [];
	const names = /* @__PURE__ */ new Set();
	for (const match of stderr.matchAll(PNPM_GIT_PACKAGE_PATTERN)) if (match[1]) names.add(match[1]);
	return [...names];
}
/**
...
*/
async function allowPnpmGitPackageBuilds(dshHome, profile, packageNames) {
	...
	const allowed = new Set(Array.isArray(existing) ? existing.filter((name) => typeof name === "string") : []);
...
var DshLauncher = class {
	...
	async install(dshHome, profile, spec, cwd, signal, options) {
		...
		for (let attempt = 0;; attempt += 1) {
			...
			if (blocked.length > 0 && await allowPnpmGitPackageBuilds(dshHome, profile, blocked)) continue;
...
async function enableBuiltinMount(input) {
	...
	const bundled = (await listBundledOptInPackages(input.bundledRoot)).find((entry) => entry.packageName === packageName);
	if (!bundled) throw new EvolutionError("not_found", "The built-in capability is no longer bundled with this Host", { packageName });
	if (bundled.version !== version) throw new EvolutionError("review_expired", "The built-in capability version changed between selection and enablement", {
		expectedVersion: version,
		actualVersion: bundled.version
	});
	if (bundled.mountId !== mountId) throw new EvolutionError("review_expired", "The built-in capability mount id changed between selection and enablement", {
...
function authorizationForResolution(resolution, reviews = []) {
	if (resolution.schemaVersion !== 2 || resolution.policyVersion !== "10" || !resolution.authorization) return {
		state: "selection_required",
...
async function preserveCancelledManagedWork(deps, input) {
	let checkpoint;
	try {
...
async function finishManagedWork(deps, resolution, exec, workflow) {
	...
	try {
		...
		const baselineBlockers = modificationBlockers(workflow.modificationOutcome ? await deps.store.getReview(workflow.modificationOutcome.baselineReviewId) : baselineReview);
...
async function scanOrphanedInstallations(deps) {
	...
	const tracked = new Set((await deps.store.listInstallations()).filter((record) => !record.removed && record.packageName).map((record) => record.packageName));
	const orphans = [];
	for (const candidate of candidates) {
...
async function adoptInstallation(deps, input) {
	...
	const tracked = (await deps.store.listInstallations()).find((record) => !record.removed && record.packageName === packageName);
...
function semanticState(view) {
	...
	if (workflow.cursor === "await_selection") return (workflow.candidateSnapshot?.length ?? 0) === 0 ? "no_candidates" : "waiting_candidate_selection";
...
function lifecycleStateFor(workflow, extras = {}) {
	if (!(workflow.policyVersion !== "10" && workflow.status === "completed" && COMPLETED_CLEANUP_NODES.has(workflow.cursor)) && workflow.policyVersion !== "10" || workflow.lastFailure?.code === "policy_restart_required") return "interrupted";
...
var WorkflowEngineCore = class {
	...
	assertOwner(workflow, exec) {
		...
		if (workflow.policyVersion !== "10") throw new EvolutionError("invalid_input", "Workflow predates the current policy and cannot be controlled");
	}
	...
	async reissueInterrupt(workflow, exec) {
		...
		const managedActionsAvailable = workflow.cursor === "await_confirmation" ? await this.host.managedWorkAvailable?.(exec) ?? true : true;
...
var WorkflowEngineDriver = class extends WorkflowEngineCore {
	...
	async supersedePendingClarifications(sessionId, cwd, currentTurnId, supersedingWorkflowId, exec) {
		...
		const pending = (await this.store.listWorkflows()).filter((item) => item.policyVersion === "10" && item.ownerSessionId === sessionId && item.cwd === cwd && item.status === "interrupted" && item.cursor === "await_clarification" && item.startedTurnId !== currentTurnId);
	...
	async present(input, exec) {
		return await this.withLock(input.workflowId, async () => {
			...
			if (ids.length < 1 || ids.length > 5) throw new EvolutionError("invalid_input", "Present requires one to five discovery candidate_ids");
...
var WorkflowEngineRecovery = class extends WorkflowEngineDriver {
	async recover(input, exec) {
		...
		const lockedView = await this.withLock(input.workflowId, async () => {
			const workflow = await this.store.getWorkflow(input.workflowId);
			this.assertSameOwner(workflow, exec);
			if (workflow.policyVersion !== "10" && !this.isCompletedCleanup(workflow)) {
	...
	async recoverCompletedInstallation(workflow, input, exec, setRestart) {
		...
		if (workflow.policyVersion !== "10" && !this.creationGuard.lastUserMessage(exec.agent)) throw new EvolutionError("invalid_input", "Legacy completed-install cleanup requires the current top-level user message before a fresh Policy V10 workflow can start");
	...
	async finishCleanupAndRestart(workflow, exec, input, setRestart) {
		const sessionId = ownerSessionId(exec.agent);
		const legacy = workflow.policyVersion !== "10";
...
var CapabilityEvolutionService = class {
	...
	versionTrackingDeps() {
		return {
			...
			config: this.config,
			launcher: this.launcher,
			createRollbackInstaller: () => new PluginInstaller(this.ctx, this.config, this.store, this.launcher, (review, signal) => this.revalidate(review, signal), void 0, void 0, void 0, ISOLATED_VERIFICATION_PROFILE, () => this.currentProfileOwner())
		};
	}
	...
	async enableBuiltin(workflow, exec) {
		...
		if (!commitment || commitment.requestedAction !== "enable_builtin" || endpoint?.kind !== "host_bundled_enable") throw new EvolutionError("invalid_input", "enable_builtin requires a frozen host-bundled enablement commitment");
...
function presentResumePendingCard(args) {
	...
	if (navKind === "enable_builtin") return genericPendingCard(args, "Enabling the built-in Host capability", "正在启用内置能力", "execute");
