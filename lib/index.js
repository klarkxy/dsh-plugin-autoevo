...
function nextStepForAuthorization(requirement, authorization) {
	...
	if (authorization.state === "use_review") return zh ? "用户选择使用这次审查的插件。去安装，不要另建一个替代品。" : "The user chose this reviewed plugin. Install it; do not create a replacement.";
	if (authorization.state === "modify_review") return zh ? "用户选择在这次审查上做最小修改，然后对本地检出再审。" : "The user chose to improve this review. Modify it minimally, then review the local checkout.";
...
var CreationGuard = class {
	...
	assertInstallAuthorized(agent, review) {
...
const POLICY_VERSION = "v5-2026-08-17";
const TOOL_NAMES = [
	"capability_resolve",
	"capability_decide",
	"plugin_review",
...
var PluginInstaller = class {
	...
	async install(input, exec) {
		...
		if (review.manifest.kind !== "bundle" || review.fit !== "full" || review.compatibility.status === "incompatible" || !sourceCanInstall || review.findings.some((finding) => finding.code === "review_truncated") || !userChoseUse && (review.recommendation !== "use" || review.securityRisk === "high")) throw new EvolutionError("review_rejected", "This review does not authorize installation", {
...
function authorizationForResolution(resolution, reviews = []) {
	if (resolution.schemaVersion !== 2 || resolution.policyVersion !== "v5-2026-08-17" || !resolution.authorization) return {
		state: "selection_required",
...
var CapabilityEvolutionService = class {
	...
	constructor(ctx, config, runner, store, creationGuard) {
		...
		this.installer = new PluginInstaller(ctx, config, store, this.launcher, (review, signal) => this.revalidate(review, signal), (review, exec) => this.creationGuard.assertInstallAuthorized(exec.agent, review));
	...
	async review(input, exec) {
		...
		if (input.sourceKind === "github") {
			...
			const current = authorizationForResolution(resolution, await this.store.listReviews(resolution.id));
...
5. After reviewing a selected plugin, explain fit, risk, missing pieces, and findings in chat. Do not call ask_user. Wait for the user's reply, then call capability_decide (use this, improve it, create new, or stop). scratch_ready means the user allowed one new plugin, not "start building".
6. Prefer reuse; when the user chooses to improve a partial candidate, extend it minimally instead of replacing it.
