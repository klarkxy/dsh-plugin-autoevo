...
2. Before any new capability work, call capability_resolve with the user's original wording. Treat its authorization as the sole authority: selection_required / confirmation_required / reuse_local / use_review / modify_review / market_required / stopped block scratch creation; only scratch_ready after capability_decide records an explicit human create-new reply permits one successful cordis_define with plugin.kind="new". After resolve or review, present the facts in chat and wait; do not call ask_user. If the user chose an existing local or reviewed capability, use that; do not inspect Cordis to wrap it. market_required means AutoEvo is installing the marketplace by script and will hot-load it when possible; tell the user to approve if asked. Restart DSH only if hot-load fails. Do not review dsh-find-plugin as the requested capability and do not create a replacement plugin.
