...
- 本地能力不足时，优先调用当前 Agent scope 内已有的 [`find_dsh_plugin`](https://github.com/awesome-dsh-plugin/dsh-find-plugin)；仅当它缺失、失败或没有有效结果时，才用已认证的 `gh` 做有界 GitHub 搜索。两条发现路径都从 `dsh-plugin` topic 取候选，再由 Agent rerank。
...
它应先调用 `capability_resolve`。如果当前 scope 安装并开放了 `find_dsh_plugin`，AutoEvo 会优先复用它；否则 GitHub 搜索会使用本机已登录的 `gh`。
