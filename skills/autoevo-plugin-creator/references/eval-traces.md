...
| Scratch | `capability_workflow` -> present shortlist in chat -> user create-new -> `capability_workflow_resume` -> `create_authorized` -> current-session managed source construction/checks -> `finish_managed_work` -> re-review -> fresh install decision -> Host verification | `cordis_define(kind:new)`, construction before the user allows create-new, silently changing target, or popping `ask_user` instead of chatting |
