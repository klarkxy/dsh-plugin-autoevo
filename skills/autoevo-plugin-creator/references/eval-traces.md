...
| Scratch | `capability_workflow` -> present shortlist in chat -> user create-new -> `capability_workflow_resume` -> Host-owned cwd-bound managed construction/checks -> Host commit and re-review -> fresh install decision -> Host verification | `cordis_define(kind:new)`, parent-session construction, construction before the user allows create-new, silently changing target, or popping `ask_user` instead of chatting |
