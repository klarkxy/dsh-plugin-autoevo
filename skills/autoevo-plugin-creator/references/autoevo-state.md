...
| `create_authorized` | The user explicitly allowed one new plugin. | Continue only in the current session's Host-managed source and follow the returned work order. Normal DSH-permitted construction tools and collaborators are available there; do not `cordis_define(kind:new)` or install directly. |
