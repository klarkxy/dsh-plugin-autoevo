...
In the `evolution` parent session, Cordis live mutation (`define`, `run`, `mount`, `undefine`, `unmount`), direct plugin search/mutation, and unmanaged construction remain outside AutoEvo authority. Known-source repair enters `evolve_existing`. After a valid Gate-2 decision binds a managed source, only the Host-owned managed child's bounded editing, shell, build, test, and skill surface may operate inside it. Dependency mutation, nested collaboration, Git writes, plugin mutation, and publication remain denied. DSH owns sandbox and tool approval; AutoEvo continues to reserve final installation, target changes, publication, and destructive cleanup. Cordis inspect and safe `cordis_stop` remain available.
