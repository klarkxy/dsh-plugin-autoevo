...
After install, AutoEvo creates the **Capability Evolution** user preset (id `evolution`, template V13) from scratch. Official Creator remains available as a user-switchable preset; AutoEvo governs. Capability Evolution does not inherit Creator mode. AutoEvo owns community-plugin reuse, review, install, and upgrade; create, modify, and correction construction happen in the current Capability Evolution session on a managed source. Host no longer launches child Agents and never falls back to `code`. An improved plugin can be contributed upstream after explicit approval. There is no legacy preset migration path: the package trusts only the exact current V13 content and preserves every other existing or edited preset instead of overwriting it. Config `evolutionPreset` defaults to `true`; `false` skips installation and never deletes an existing preset. Workflows, reviews, and install receipts remain under `<dshHome>/autoevo/` so existing Host ledgers stay visible; managed git sources default to `.autoevo/sources/` in the current session workspace. Plugins already installed into a DSH profile do not depend on workspace source files. Runtime policy is Policy V8. A completed installation can be cleaned up and restarted only after a new top-level user request, using that workflow's owned receipt. Failed `recovery_required` still uses the sealed interrupt protocol; the two paths must not be mixed.
...
To create a new dynamic Cordis plugin, start or switch a blank/new session to Capability Evolution. Official Creator remains for existing-plugin repair and static development. AutoEvo governs discovery, review, authorization, install, and in-session managed-source construction. AutoEvo does not replace the shipped `cordis-plugin-development` skill globally.
...
- The parent session governs discovery, review, authorization, install, and in-session create/modify construction after the user confirms. The execution guard denies `cordis_define(kind:new)`, unreviewed direct plugin install/remove, and subagent/workflow/ralph. Host no longer creates child Agents and does not mount official Creator onto a hidden session. Official `cordis` Creator files are unchanged. Construction sources live under `.autoevo/sources/` in the current workspace, while Host receipts remain under `<dshHome>/autoevo/` (override with `sourceDir` / `stateDir`); neither path falls back to `code`. On Windows, sandbox enforcement is integrity-oriented partial isolation.
...
- Cancelling the parent task has no child Agent to dispose. Bounded edits are checkpointed under an independent cleanup lifetime, the workflow parks at `recovery_required`, and source locks are released; cancellation is never reported as a missing executable.
