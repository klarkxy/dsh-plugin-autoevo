...
- The parent session governs discovery, review, authorization, install, and in-session create/modify construction after the user confirms. The execution guard denies `cordis_define(kind:new)`, unreviewed direct plugin install/remove, and subagent/workflow/ralph. Host no longer creates child Agents and does not mount official Creator onto a hidden session. Official `cordis` Creator files are unchanged. Construction uses a managed git source under `sourceDir` (default `<stateDir>/sources`) and never falls back to `code`. On Windows, sandbox enforcement is integrity-oriented partial isolation.
