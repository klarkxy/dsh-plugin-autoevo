...
2. manifest 精确声明 `dsh.bundle.patch`，package name 通过 registry-name 校验；
3. fit 为 `full`，recommendation 为 `use`；
4. 风险为 `low` 或 `medium`，DSH 兼容性为 `compatible` 或 `unknown`；
...
symlink、特殊文件或截断的本地快照停在审查阶段。材料变化记为 `review_expired`。风险 `high` 或兼容性 `incompatible` 的候选留在审查结果里。
...
验证器核对 callId/name 匹配、结果成功、预期工具全部覆盖，以及 DSH 任务输出。任务输出只保存 SHA-256。

## 5. 删除

临时目录由 `installationId` 唯一拥有。删除前对 trial root 与候选路径做 `realpath`，确认候选是 trials root 的严格子目录。删除使用 Node `rm`，对象是已经验证的精确路径。

外部安装前先持久化 provisional receipt。最终 receipt 写入失败时，temporary trial 立即补偿删除；persistent Profile 保留 provisional recovery anchor。清理 persistent 失败记录时先查 Profile 依赖；依赖已经消失就只删 owned artifact。临时试用在批准和文件写入前就确定验证任务。
